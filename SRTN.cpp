   ...
   void updatepcb(int newstate,int curtime)
   {
    ...
    else if(newstate==1) //resumed process
    {
      ...
      totalwait+=(executionStart-laststopTime);
       ...
       remainingTime=runtime-(curtime-executionStart);
        }
     else
     {
       finishTime=curtime;
...
public:
    ...
    fptr=fopen(fname,"a");
     ...
     { rewind(fptr);
  fprintf(fptr, "At time %d process %d %s arr %d total %d remain %d wait %d TA %d WTA %.2f \n",process.executionStart,process.id,states[process.state],process.arrivalTime,process.runtime,process.remainingTime,wait,ta,wta);
fclose(fptr);
}
          else if(fptr!=NULL)
          {  rewind(fptr);
 fprintf(fptr, "At time %d process %d %s arr %d total %d remain %d wait %d \n",process.executionStart,process.id,states[process.state],process.arrivalTime,process.runtime,process.remainingTime,wait);
 fclose(fptr);
}
...
int main(int argc, char* argv[]) {
 ...
 ///////you have to check if there is initially running processs////////////////////////////////
     ...
     if(!(running)||(newprocess.remainingTime<runningprocess.remainingTime))
     {
